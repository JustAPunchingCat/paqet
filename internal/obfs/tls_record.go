...
func NewTLSRecordObfuscator(cfg *conf.Obfuscation, key []byte) (Obfuscator, error) {
	initNoise()
...
func (o *TLSRecordObfuscator) Overhead() int {
	return tlsRecordHeaderSize + 2 + o.maxPad // Header + length encoding + max padding
}
...
func (o *TLSRecordObfuscator) wrapWithLength(data []byte) ([]byte, error) {
	...
	if padLen > 0 {
		prngMu.Lock()
		start := prng.Intn(len(noise) - padLen)
		prngMu.Unlock()
		copy(result[tlsRecordHeaderSize+2+dataLen:], noise[start:start+padLen])
	}
