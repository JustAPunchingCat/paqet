...
// Fast, thread-safe PRNG for non-crypto randomization (headers, padding lengths)
var (
...
func RandInRange(min, max uint32) uint32 {
	...
	diff := max - min + 1
	return min + (fastRandUint32() % diff)
}
...
func RandInRange16(min, max uint16) uint16 {
	...
	return uint16(uint32(min) + (fastRandUint32() % diff))
}
...
func RandInRange8(min, max uint8) uint8 {
	...
	diff := uint32(max) - uint32(min) + 1
	return uint8(uint32(min) + (fastRandUint32() % diff))
}
...
func GenerateRealisticTOS() uint8 {
	...
	idx := fastRandUint32() % uint32(len(tosValues))
...
func GenerateRealisticWindow() uint16 {
	...
	idx := fastRandUint32() % uint32(len(windowSizes))
...
func GenerateRealisticMSS() uint16 {
	...
	idx := fastRandUint32() % uint32(len(mssValues))
...
func GenerateRealisticWindowScale() uint8 {
	...
	idx := fastRandUint32() % uint32(len(scales))
	return scales[idx]
}
...
func AddJitter(value uint32, jitterPercent float64) uint32 {
	...
	jitter := fastRandUint32() % (2*maxJitter + 1)
...
func ShuffleTCPOptions(options []interface{}) []interface{} {
	...
	for i := n - 1; i > 0; i-- {
		j := int(fastRandUint32() % uint32(i+1))
